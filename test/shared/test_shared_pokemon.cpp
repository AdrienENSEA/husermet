...
{
  {
    ...
    BOOST_CHECK_EQUAL(groudon.getItem().getName(), restes.getName());
...

  {
    ...
    StatsAttack stats_attack {0, 0, 0, 0, 0, 0};
    ModifStatsPokemon stats_modif;
    Attack a1(PIEGE_DE_ROC, "Piege de Roc", "desc", ROCK, stats_attack, stats_modif,0);
    std::vector <Attack> attacks {a1};
    Item ballon(BALLON, "Ballon", "description", effect_stats, NONE_S, FLY);
    Pokemon heatran;
    heatran.setName("Heatran");
    heatran.setItem(ballon);
    heatran.setAttack(0, a1);
    BOOST_CHECK_EQUAL(heatran.getName(), "Heatran");
    std::vector <Attack> a = heatran.getAttacks();
    BOOST_CHECK(!a.empty());
    BOOST_CHECK_EQUAL(a.at(0).getIdAttack(), PIEGE_DE_ROC);
    BOOST_CHECK_EQUAL(a.at(0).getName(), "Piege de Roc");
    BOOST_CHECK_EQUAL(heatran.getItem().getIdItem(), BALLON);
  }
}
